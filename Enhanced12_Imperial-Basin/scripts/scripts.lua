InfantryTypes = {"light_inf", "trooper", "engineer", "thumper", "grenadier", "fremen", "fremen.spawn", "sardaukar", "mpsardaukar", "sardaukar.level1", "sardaukar.level2", "sardaukar.level3", "saboteur", "worm_rider"}
MicroUnitTypes = {"trike", "raider", "stealth_raider", "sand_runner", "quad", "heavy_quad", "dustrider"}
TankTypes = {"dustrider", "combat_tank_a", "combat_tank_h", "combat_tank_o", "bison_tank"}
...
WorldLoaded = function()
	...
	Trigger.OnAnyProduction( function(producer, produced, productionQueue)
		...
		if Utils.Any(TankTypes, function (t) return t == actor end) then
			autoCrush(produced, produced.Owner)
		end

		if productionQueue == "Armor" or productionQueue== "Vehicle" then -- productionQueue == "Aircraft"
			vehicleRepair(produced, produced.Owner.InternalName)
		end
	end)
...
function autoCrush(unit, bot)
	...
	local targets = Utils.Where(actors, function(a)
		return Utils.Any(InfantryTypes, function (t) return t == a.Type end)
	end)
	if next(targets) ~= nil then
...
function botCarryallCheck()
	for _, player in ipairs(ActivePlayers) do
		if player.IsBot then
			...
			if #carryalls < #harvesters / 2 then
